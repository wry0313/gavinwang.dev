...
In addition, while the time tracker function in background.js is working, it is not working correctly enough. It only records the website titles when I refresh the page. And because nowadays all websites are rendered by JavaScript, when I click a new link within a website, the function wouldn't record any title change. However, ChatGPT also has the capability to fix that. I simply told it about the code's problem, and ChatGPT gave me a revised version that worked exactly as I wanted (I still have to make some small adjustments, but those are easy to do):
